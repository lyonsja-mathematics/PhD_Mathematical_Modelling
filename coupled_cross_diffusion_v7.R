...
coupled_cross_diffusion_v7 = function(
  ...
  n_interations = total.time*iterations.per.second +1# Total number of iterations
  ...
  df.m %>% melt(id.vars=c("x")) %>% rename(time=variable, m=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time)) %>% arrange(time,x) -> df.m# Additional columns containing info on input
  ...
  df.p %>% melt(id.vars=c("x")) %>% rename(time=variable, p=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time)) %>% arrange(time,x) -> df.p# Additional columns containing info on input
  ...
  df.q %>% melt(id.vars=c("x")) %>% rename(time=variable, q=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time)) %>% arrange(time,x) -> df.q# Additional columns containing info on input
  ...
  df.z %>% melt(id.vars=c("x")) %>% rename(time=variable, z=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time),z=z0*z) %>% arrange(time,x) -> df.z# Additional columns containing info on input
  ...
  df = cbind(df.m,p=df.p$p,q=df.q$q,z=df.z$z,n=df.n$n)# %>% subset(time %in% time.vals)
  ...
  for(k in 1:Nt){# For each time point ...
    ...
    Q0[k]=Delta.x/3*(q0[1] + q0[Nx] + sum(2*q0[times.2]) + sum(4*q0[times.4]))
    Q1[k]=2*Delta.x/3*(q1[1] + q1[Nx] + sum(2*q1[times.2]) + sum(4*q1[times.4]))
  ...
  redistributed=c()
  for(i in time){
    df %>% subset(time==i) %>% arrange(x) %>% mutate(v1=z/z0) %>% pull(v1) -> zi
  ...
  df.Dn=data.frame(time, shrinkage, distortion, Delta.n, redistributed, N0, Q1)
  df=merge(df, df.Dn, by=c("time"))
  
  df %>% mutate(end_exp=end_exp,total.time,lpmm,I0,xi,Dm,nm,rhom,rhop,Dp,np,nq,Gamma,wt.pc,Dz,epsilon.mz,epsilon.pz,epsilon.qz,rhoz,nz,nb,rhob,b0,z0,T0,T.t=T0*(1-shrinkage),alpha.m,beta,Delta.t,Delta.x,output.time.step,theta.B.t=asin(lambda.probe/2/N0/Lambda),nu=pi*Delta.n*T.t/lambda.probe/cos(theta.B.t),Model="coupled_cross_diffusion_v7") -> df
