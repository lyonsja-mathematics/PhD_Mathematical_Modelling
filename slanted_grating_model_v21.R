...
slanted_grating_simulation_v21 = function(
  ...
  slant.angle=1e-4,# Grating slant angle
  ...
  epsilon.pz=13,# Cross-diffusion ratio
	epsilon.qz=13,# Cross-diffusion ratio
  ...
  for(j in 1:Nt){
    ...
    for(i in 1:Nx){
      ...
      f[,i] = exp(-0.3*zeta_star*u1*(1-Y[i]))*(1 + exp(-xi_star*z1.i)*cos(2*pi*x - 2*pi*T0/y.hat1*u1*Y[i]))
    }
    f=melt(f)$value
    ...
    QQ2=diag(2, Nx*Nx)
    QQ1=diag(2, Nx*Nx)
    QP2=diag(+Phi*gamma*Delta.t*as.numeric(p1),Nx*Nx)
    QP1=diag(+Phi*gamma*Delta.t*as.numeric(p1),Nx*Nx)
    ZZ2 = diag(2, Nx*Nx)
    ZZ1 = diag(2, Nx*Nx)
    BB2 = diag(2, Nx*Nx)
    BB1 = diag(2, Nx*Nx)
    ...
    for(i in 1:(Nx*Nx)){
      ...
      MM2[i, i.minus.1] = MM2[i, i.minus.1] - r*alpha.m.x
	    MM2[i, j.minus.1] = MM2[i, j.minus.1] - r*alpha.m.y/u1/u1
	    MM2[i, i] = MM2[i, i] + 2*r*alpha.m.x
	    MM2[i, i] = MM2[i, i] + 2*r*alpha.m.y/u1/u1
	    MM2[i, i.plus.1] = MM2[i, i.plus.1] - r*alpha.m.x
	    MM2[i, j.plus.1] = MM2[i, j.plus.1] - r*alpha.m.y/u1/u1
	    
	    MM1[i, i.minus.1] = MM1[i, i.minus.1] + r*alpha.m.x
	    MM1[i, j.minus.1] = MM1[i, j.minus.1] + r*alpha.m.y/u1/u1
	    MM1[i, i] = MM1[i, i] - 2*r*alpha.m.x
	    MM1[i, i] = MM1[i, i] - 2*r*alpha.m.y/u1/u1
	    MM1[i, i.plus.1] = MM1[i, i.plus.1] + r*alpha.m.x
	    MM1[i, j.plus.1] = MM1[i, j.plus.1] + r*alpha.m.y/u1/u1
      ...
      ZZ2[i, i.minus.1] = ZZ2[i, i.minus.1] - r*alpha.z.x*(1 + epsilon.qz*q1[i.minus.1] + epsilon.pz*p1[i.minus.1])
	    ZZ2[i, j.minus.1] = ZZ2[i, j.minus.1] - r*alpha.z.y/u1/u1*(1 + epsilon.qz*q1[j.minus.1] + epsilon.pz*p1[j.minus.1])
	    ZZ2[i, i] = ZZ2[i, i] + 2*r*alpha.z.x*(1 + epsilon.qz*q1[i] + epsilon.pz*p1[i])
	    ZZ2[i, i] = ZZ2[i, i] + 2*r*alpha.z.y/u1/u1*(1 + epsilon.qz*q1[i] + epsilon.pz*p1[i])
	    ZZ2[i, i.plus.1] = ZZ2[i, i.plus.1] - r*alpha.z.x*(1 + epsilon.qz*q1[i.plus.1] + epsilon.pz*p1[i.plus.1])
	    ZZ2[i, j.plus.1] = ZZ2[i, j.plus.1] - r*alpha.z.y/u1/u1*(1 + epsilon.qz*q1[j.plus.1] + epsilon.pz*p1[j.plus.1])
	    
	    ZZ1[i, i.minus.1] = ZZ1[i, i.minus.1] + r*alpha.z.x*(1 + epsilon.qz*q1[i.minus.1] + epsilon.pz*p1[i.minus.1])
	    ZZ1[i, j.minus.1] = ZZ1[i, j.minus.1] + r*alpha.z.y/u1/u1*(1 + epsilon.qz*q1[j.minus.1] + epsilon.pz*p1[j.minus.1])
	    ZZ1[i, i] = ZZ1[i, i] - 2*r*alpha.z.x*(1 + epsilon.qz*q1[i] + epsilon.pz*p1[i])
	    ZZ1[i, i] = ZZ1[i, i] - 2*r*alpha.z.y/u1/u1*(1 + epsilon.qz*q1[i] + epsilon.pz*p1[i])
	    ZZ1[i, i.plus.1] = ZZ1[i, i.plus.1] + r*alpha.z.x*(1 + epsilon.qz*q1[i.plus.1] + epsilon.pz*p1[i.plus.1])
	    ZZ1[i, j.plus.1] = ZZ1[i, j.plus.1] + r*alpha.z.y/u1/u1*(1 + epsilon.qz*q1[j.plus.1] + epsilon.pz*p1[j.plus.1])

    	MM2[i, j.minus.1] = MM2[i, j.minus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	MM1[i, j.minus.1] = MM1[i, j.minus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	MM2[i, j.plus.1] = MM2[i, j.plus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	MM1[i, j.plus.1] = MM1[i, j.plus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt

    	PP2[i, j.minus.1] = PP2[i, j.minus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	PP1[i, j.minus.1] = PP1[i, j.minus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	PP2[i, j.plus.1] = PP2[i, j.plus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	PP1[i, j.plus.1] = PP1[i, j.plus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	
    	QQ2[i, j.minus.1] = QQ2[i, j.minus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	QQ1[i, j.minus.1] = QQ1[i, j.minus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	QQ2[i, j.plus.1] = QQ2[i, j.plus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	QQ1[i, j.plus.1] = QQ1[i, j.plus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt

    	ZZ2[i, j.minus.1] = ZZ2[i, j.minus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	ZZ1[i, j.minus.1] = ZZ1[i, j.minus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	ZZ2[i, j.plus.1] = ZZ2[i, j.plus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	ZZ1[i, j.plus.1] = ZZ1[i, j.plus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	
    	BB2[i, j.minus.1] = BB2[i, j.minus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	BB1[i, j.minus.1] = BB1[i, j.minus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	BB2[i, j.plus.1] = BB2[i, j.plus.1] - Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    	BB1[i, j.plus.1] = BB1[i, j.plus.1] + Y1[i]/u1*Delta.t/2/Delta.x*du.dt
    ...
    du.dt = (int_dmdt/rhom + int_dpdt/rhop + int_dqdt/rhop + int_dzdt/rhoz)/Volume0
  ...
  df.m %>% melt(id.vars=c("x","Y")) %>% rename(time=variable, m=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time)) %>% arrange(time,Y,x) -> df.m# Additional columns containing info on input
  ...
  df.p %>% melt(id.vars=c("x","Y")) %>% rename(time=variable, p=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time)) %>% arrange(time,Y,x) -> df.p# Additional columns containing info on input
  ...
  df.q %>% melt(id.vars=c("x","Y")) %>% rename(time=variable, q=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time)) %>% arrange(time,Y,x) -> df.q# Additional columns containing info on input
  ...
  df.z %>% melt(id.vars=c("x","Y")) %>% rename(time=variable, z=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time),z=z0*z) %>% arrange(time,Y,x) -> df.z# Additional columns containing info on input
  ...
  df.b %>% melt(id.vars=c("x","Y")) %>% rename(time=variable, b=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time),b=b0*b) %>% arrange(time,Y,x) -> df.b# Additional columns containing info on input
  ...
  data.frame(x=rep(seq(0,1,Delta.x), Nx), Y=sort(rep(seq(0,1,Delta.x), Nx)), n) -> df.n# Turn the matrix into a data frame.
  names(df.n)=c("x","Y", paste0("t",time.vals[1:ncol(n)]))# Rename columns
  df.n %>% melt(id.vars=c("x","Y")) %>% rename(time=variable, n=value) %>% mutate(time=gsub(x=time, pattern="t", replacement=""), time=as.numeric(time)) %>% arrange(time,Y,x) -> df.n# Additional columns containing info on input
  
  cbind(df.b,m=df.m$m,p=df.p$p,q=df.q$q,z=df.z$z,n=df.n$n) -> df1
  ...
  for(k in 1:Nt){
    ...
    matrix(n[, k], nrow=Nx) -> n1
  	...
  	Mean.RI[k]=Delta.x*Delta.x/4*(n1[1,1] + n1[Nx,1] + n1[1,Nx] + n1[Nx,Nx] + sum(2*n1[1,(2:(Nx-1))]) + sum(2*n1[(2:(Nx-1)),1]) + sum(2*n1[Nx,(2:(Nx-1))]) + sum(2*n1[(2:(Nx-1)),Nx]) + sum(4*n1[(2:(Nx-1)),(2:(Nx-1))]) )
  ...
  data.frame(
    ...
    Y=sort(rep(seq(0,1,Delta.x), Nt)),
    Delta.n=melt(Delta.n)$value,
    delta=melt(delta)$value
  ) -> df2
  
  df1 %>% mutate(Delta.n=0, delta=0) %>% arrange(time,Y,x) -> df1
  
  for(i in time){
  	for(j in Y){
  		df1[df1$time==i & df1$Y==j, "Delta.n"] = df2[df2$time==i & df2$Y==j, "Delta.n"]
  ...
  data.frame(
    ...
    Lambda.t,
  	...
  	theta_B=asin(lambda.probe/2/Mean.RI/Lambda.t) - phi.r.t,
  ...
  for(i in time){
  	for(j in c("T.t","Mean.RI","Actual.Shrinkage","Lambda.t","phi.r.t","int_M","int_P","int_Q","int_Z","int_B","theta_B","Delta_theta_B","Delta_phi_r","Apparent.Shrinkage")){
  		df1[df1$time==i, j] = df3[df3$time==i, j]
  	}
  }
  ...
  df1 %>%
    mutate(
      ...
      zeta,
    	lambda.probe,
    	nu=pi*Delta.n*T.t/lambda.probe/cos(theta_B),
    	Moharam.Young=lambda.probe*lambda.probe/Mean.RI/Delta.n/Lambda.t/Lambda.t/cos(phi.r.t),
    	Klein.Cook=2*pi*lambda.probe*T.t/Mean.RI/Lambda.t/Lambda.t/cos(phi.r.t),
      Delta.t,
      Delta.x,
      output.time.step,
  ...
  if(df1 %>% subset(Y==0 & time==max(time)) %>% distinct(Klein.Cook) %>% pull(Klein.Cook) < 10){
	
		df1 %>% mutate(J0=nu/2, J1=J0) -> df1
